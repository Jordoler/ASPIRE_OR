...
def process(connection, config, metadata):
    ...
    # if replace_grappa and send_original:
    #     send_original = False
    ...
    mag_data_5d = np.zeros((width, height, slices, echo_numbers, number_of_coils), dtype=np.float32)
    ph_data_5d  = np.zeros((width, height, slices, echo_numbers, number_of_coils), dtype=np.float32)
    ...
    try:
        for item in connection:
            ...
            # ----------------------------------------------------------
                ...
                logging.debug("Sort_debug: exporting mag_5d.nii...")
                here = perf_counter() if log_time else None
                sort.data5d_to_Nifti(mag_data_5d, "mag")
                sort.log_time(log_time,here,"exporting mag_5d.nii took")
                
                logging.debug("Sort_debug: Converting phase from int to radians...")
                here = perf_counter() if log_time else None
                ph_data_5d = sort.ph5d_to_radians(ph_data_5d, phase2pi_scale)
                sort.log_time(log_time, here, "phase int to radians took")

                logging.debug("Sort_debug: exporting ph_5d.nii ...")
                here = perf_counter() if log_time else None
                sort.data5d_to_Nifti(ph_data_5d, "ph")
                sort.log_time(log_time,here,"export ph_5d.nii took")

                # running julia MCPC algorithm
                TEs_jl = sort.format_TEs(TEs)
                here = perf_counter() if log_time else None
                subprocess.run(["julia","custom_scripts/MCPC-3D-S_u.jl","--mag_file","mag_5d.nii","--phase_file","ph_5d.nii","--TEsList",*TEs_jl],check=True)
                sort.log_time(log_time,here,"julia script took")

                logging.debug("Sort_debug: processing combined mag images...")
                here = perf_counter() if log_time else None
                combined_mag = process_combined_images_opt("mag",currentSeries,mag_meta_templates,config)
                ...
                here = perf_counter() if log_time else None
                combined_ph = process_combined_images_opt("ph",currentSeries,ph_meta_templates,config)
                ...
                logging.debug("Sort_debug: Config: MCPC_uncompiled")
                break
...
def process_combined_images_opt(echo_type: str,currentSeries, meta_templates,config):
    phase2pi_scale = bool(mrdhelper.get_json_config_param(config, 'phase2piScale', default=True, type='bool'))

    logging.debug(f"Sort_debug: loading combined {echo_type} images from nifti...")
    combined_4d = nib.load("combined_"+echo_type+"_4d.nii").get_fdata() # [x,y,z,echo]
    combined_4d = combined_4d.transpose((3,2,1,0)) # [echo,z,y,x]
    combined_4d = np.nan_to_num(combined_4d, nan=0.0)
